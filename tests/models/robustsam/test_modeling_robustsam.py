...
"""Testing suite for the PyTorch RobustSAM model."""
...
from transformers import RobustSamConfig, RobustSamMaskDecoderConfig, RobustSamPromptEncoderConfig, RobustSamVisionConfig, pipeline
...
class RobustSamPromptEncoderTester:
    def __init__(
        self,
        hidden_size=32,
        input_image_size=24,
    ...
    def get_config(self):
        return RobustSamPromptEncoderConfig(
...
# Create vit_dim: int=1024, opt = None, for RobustSAM
class RobustSamMaskDecoderTester:
    def __init__(
        self,
        hidden_size=32,
        hidden_act="relu",
        ...
        # Initilalize vit_dim, opt for RobustSAM
        self.vit_dim = vit_dim
        self.opt = opt,

    def get_config(self):
        return RobustSamMaskDecoderConfig(
            hidden_size=self.hidden_size,
            hidden_act=self.hidden_act,
...
class RobustSamModelTester:
    def __init__(
        ...
        self.prompt_encoder_tester = RobustSamPromptEncoderTester()
        self.mask_decoder_tester = RobustSamMaskDecoderTester()
    ...
    def create_and_check_model(self, config, pixel_values):
        model = RobustSamModel(config=config)
...
@require_torch
class RobustSamModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as RobustSAM's vision encoder does not use input_ids, inputs_embeds,
    attention_mask and seq_length.
    """

    all_model_classes = (RobustSamModel,) if is_torch_available() else ()
    pipeline_model_mapping = (
        {"feature-extraction": RobustSamModel, "mask-generation": RobustSamModel} if is_torch_available() else {}
    )
    fx_compatible = False
    test_pruning = False
    ...
    def setUp(self):
        self.model_tester = RobustSamModelTester(self)
        self.vision_config_tester = ConfigTester(self, config_class=RobustSamVisionConfig, has_text_modality=False)
        self.prompt_encoder_config_tester = ConfigTester(
            self,
            config_class=RobustSamPromptEncoderConfig,
            has_text_modality=False,
            num_attention_heads=12,
            num_hidden_layers=2,
        )
        self.mask_decoder_config_tester = ConfigTester(
            self, config_class=RobustSamMaskDecoderConfig, has_text_modality=False
        )
    ...
    @unittest.skip(reason="RobustSAM's vision encoder does not use inputs_embeds")
    ...
    @unittest.skip(reason="RobustSamModel does not support training")
    ...
    @unittest.skip(reason="RobustSamModel has no base class and is not available in MODEL_MAPPING")
...
@slow
class RobustSamModelIntegrationTest(unittest.TestCase):
    def tearDown(self):
        super().tearDown()
