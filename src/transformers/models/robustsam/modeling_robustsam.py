...
from .configuration_robustsam import RobustSamConfig, RobustSamMaskDecoderConfig, RobustSamPromptEncoderConfig, RobustSamVisionConfig
...
@dataclass
class RobustSamVisionEncoderOutput(ModelOutput):
    """
    Base class for robustsam vision model's outputs that also contains image embeddings obtained by applying the projection
    ...
    Args:
        ...
        encoder_features: (`tuple(torch.FloatTensor)`
            The intermediate embeddings at global attention layers
    ...
    encoder_features: Tuple[torch.FloatTensor, ...] = None
    hidden_states: Optional[Tuple[torch.FloatTensor, ...]] = None
    attentions: Optional[Tuple[torch.FloatTensor, ...]] = None

# Copied from transformers.models.sam.modeling_sam.SamImageSegmentationOutput with Sam->RobustSam, and create robust_embeddings/robust_token attributes
@dataclass
class RobustSamImageSegmentationOutput(ModelOutput):
    """
    Base class for RobustSAM model's output
...
class RobustSamTwoWayAttentionBlock(nn.Module):
    def __init__(self, config, attention_downsample_rate: int = 2, skip_first_layer_pe: bool = False):
        ...
        Arguments:
            config (`RobustSamMaskDecoderConfig`):
...
class RobustSamTwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        token_att_map = [] #create token_att_map for RobustSAM

        attn_out = self.final_attn_token_to_image(query=query, key=key, value=keys, opt = opt)
...
class LastLayerFeatureBlock(nn.Module):
    ...
    def forward(self, x, clear=True):
        ...
        output = self.upsample_layer_conv1(x)
        output = self.upsample_layer_LayerNorm2d(output)
        output = self.upsample_layer_act(output)
        output = self.upsample_layer_conv2(output)
        return output
...
class MaskFeatureBlock(nn.Module):
    ...
    def forward(self, x, clear=True):
        ...
        output = self.downsample_layer_conv1(x)
...
class RobustSamMaskDecoder(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.num_mask_tokens):
            # Process ROT when clear = False for RobustSAM
            if clear:
        ...
        # Create token_att_map for RobustSAM
        token_att_map = None
        ...
        outputs = (masks, iou_pred, upscaled_embedding_robust, robust_token) #return upscaled_embedding_robust / robust_token for RobustSAM
...
class RobustSamVisionEncoder(nn.Module):
    ...
    def forward(
        ...
        encoder_features = ()   #create encoder_features for RobustSAM
        ...
        if not return_dict:
            outputs = (hidden_states,)
            outputs = outputs + (encoder_features,) #output encoder_features for RobustSAM
            if output_hidden_states:
...
"""
# Copied from transformers.models.sam.modeling_sam.RobustSamModel with Sam->RobustSam, and Adapt SamModel to RobustSamModel
@add_start_docstrings(
    "RobustSAM for generating segmentation masks, given an input image and ",
...
class RobustSamModel(RobustSamPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not return_dict:
            output = (iou_predictions, low_res_masks, robust_embeddings, robust_token)
            if output_hidden_states:
